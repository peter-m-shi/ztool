...
# xls or strings even easier.
 ...
 Woz.configure do |config|
   ...
   config.csv_filename = "Localizable.csv"
...
TEXT
      ...
      def parse_strings list, file
        fail "!#{file} #{Woz.config.strings_filename} file not found, specify the filename in the .wozniak file" unless File.exists?(File.join(file, Woz.config.strings_filename))

        language = get_language file
        puts "# parsing #{language}"
        File.open(File.join(file, Woz.config.strings_filename), "r").each do |row|
          if row.start_with? '"'
            #Modified by peter.shi at 2015.4.14
            #Fixed bug when value include " etc. "kFlashOff" = "test\"ok\",lalalla";
            pos = row.rindex('";')            #Modified by peter.shi at 2015.4.15
            row = row[0,pos]                  #Modified by peter.shi at 2015.4.15
            if splitted = row.split(/"/, 4)   #change splits limited count to 4
      ...
      def generate_translation(type=:xls)
        puts "#using fixed version by pg_tool."
          ...
          if new_file || !Woz.config.ask_confirmation || ask("! the project's #{language} strings file will be overwritten, type 'y' and enter to continue: ") == "y"
            ...
            list[language].each do |key, value|
              unless key.nil?
                file.puts "\/\* #{key} \*\/"
                if value.nil?
                  file.puts "\"#{key}\" = \"MISSING VALUE\";"  #Modified by peter at 2015.4.14
                else
                  file.puts "\"#{key}\" = \"#{value}\";"
                end
                file.puts ""
